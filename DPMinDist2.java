public class DPMinDist2 {
    /**
     * 递归：从底向上，反过来推导，到第一个点|| 已经有结果，为递归结束条件 状态定义： minDistBT(row,col) 到row行，col列的最短路径
     * 状态转移方程：minDistBT(row,col) = min(minDistBT(row-1,col),minDistBT(row,col-1)) +
     * arr[row][col] 达到当前点的最短路径 = 从上面||左边过来中最短路径 + 当前val;
     * 
    ...
    public static void main(String[] args) {
        ...
        int result = minDistBT(arrs, states, row - 1, col - 1);
